...
function createCard(overrides: Partial<RuntimeBoardCard> = {}): RuntimeBoardCard {
	return {
		...
		prompt: "Continue implementing review feedback",
...
describe("task start command", () => {
	...
	it("given a card in review, when started, then it moves to in_progress and records that transition", async () => {
		...
		expect(inProgressCard?.transitions?.map((transition) => transition.column)).toEqual([
			"backlog",
			"in_progress",
			"review",
			"in_progress",
	...
	it.each(["done", "trash"] as const)(
		"given a card in %s, when started, then it is refused as terminal",
		async (columnId) => {
			...
			expect(trpcMocks.client.workspace.ensureWorktree.mutate).not.toHaveBeenCalled();
