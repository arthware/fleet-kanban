...
describe("CardDetailView", () => {
	...
	it("shows the external issue chip in the detail header next to the PR badge", async () => {
		...
		const issueLink = container.querySelector<HTMLAnchorElement>('a[href="https://github.com/owner/repo/issues/42"]');
		const prLink = container.querySelector<HTMLAnchorElement>('a[href="https://github.com/cline/kanban/pull/42"]');
		...
		expect(issueLink?.textContent).toContain("owner/repo#42");
		expect(issueLink?.querySelector("svg.lucide-github")).toBeInstanceOf(SVGSVGElement);
		expect(prLink).not.toBeNull();
		if (!(issueLink instanceof HTMLElement) || !(prLink instanceof HTMLElement)) {
			throw new Error("Expected issue and PR links to render.");
		}
		expect(issueLink.compareDocumentPosition(prLink) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
	});

	it("shows an unlinked external issue chip in the detail header when no URL is stored", async () => {
		const selection = createSelection();
		selection.card.externalIssue = {
			provider: "linear",
			key: "ENG-123",
		...
		expect(container.textContent).toContain("ENG-123");
		expect(container.querySelector('a[href*="linear.app"]')).toBeNull();
		expect(container.querySelector("svg.lucide-circle-dot")).toBeInstanceOf(SVGSVGElement);
	});
