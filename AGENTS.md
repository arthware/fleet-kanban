...
Card Types
- **Concept:** A card *type* is a **data manifest** (`fleet/card-types/<name>.md`) mapping ordered `phase → lane → skill(s)`. The injected directive is single-sourced from each skill's `SKILL.md` `directive:` frontmatter. Built-in `feature` = today's design→build→ship workflow.
- **When to make your own:** A recurring workflow with distinct per-phase skills (e.g. `bugfix`, `migration`, `spike`, `security-review`, `docs`) — instead of stacking a new flag.
...
- **Scope the verify gate to the change surface.** For a CLI/runtime-only change (`src/commands/*`, `src/cli.ts`, `src/**` non-web), the gate is `npm run typecheck` + `npm run test:fast` (the pre-commit tier = `vitest run test/runtime test/utilities test/agents --exclude "**/*.live.test.ts"`) + the specific test files your diff touches (`npx vitest run <file>`). Live-only tests must use the `.live.test.ts` file suffix to be automatically excluded from the fast pre-commit tier by construction. For a web-ui change, use `npm --prefix web-ui run typecheck` + targeted `web:test` files. **Never run the full `npm run build` on the inner loop** — it bundles the entire web app (vite, ~2MB) + sentry and is pure waste for a CLI change; if you need the built `dist/cli.js` to smoke-test, build only the CLI with `node scripts/build.mjs` (skips `web:build`).
