...
- `src/core/card-type.ts` / `src/prompts/card-type-discovery.ts` / `src/prompts/compose-card-directive.ts` — **card types and skill pipeline** (Zod manifest validation, project-first discovery, frontmatter-derived directive generator). Built-in manifest is at `fleet/card-types/feature.md`.
